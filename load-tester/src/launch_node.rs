...
pub fn launch_node(
    ...
    match fs::create_dir(&data_dir) {
        ...
        r => r.chain_err(|| "failed to create bitcoinblack_node data directory")?,
    }
    ...
    let child = Command::new(bitcoinblack_node)
        .arg("--data_path")
        .arg(&data_dir)
        .arg("--daemon")
        .spawn_async(&handle)
        .chain_err(|| "failed to spawn bitcoinblack_node")?;
...
pub fn connect_node<C: Connect>(
    ...
    })).then(|x| x.chain_err(|| "failed to call bitcoinblack_node RPC"))
            .map(|_| ()),
    ) as _
}
