...
TEST (rpc, history)
{
	...
	rai::genesis genesis;
	rai::state_block usend (rai::genesis_account, node0->latest (rai::genesis_account), rai::genesis_account, rai::genesis_amount - rai::Gbcb_ratio, rai::genesis_account, rai::test_genesis_key.prv, rai::test_genesis_key.pub, 0);
	...
	ASSERT_EQ (rai::Gbcb_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	...
	ASSERT_EQ (rai::Gbcb_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
...
TEST (rpc, payment_wait)
{
	...
	request1.put ("amount", rai::amount (rai::Mbcb_ratio).to_string_dec ());
	...
	system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::Mbcb_ratio);
	...
	request1.put ("amount", rai::amount (rai::Mbcb_ratio * 2).to_string_dec ());
	test_response response3 (request1, rpc, system.service);
	while (response3.status == 0)
	{
		system.poll ();
...
TEST (rpc, mbcb_to_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "mbcb_to_raw");
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::Mbcb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, mbcb_from_raw)
{
	rai::system system (24000, 1);
	rai::node_init init1;
	auto & node1 (*system.nodes[0]);
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "mbcb_from_raw");
	request1.put ("amount", rai::Mbcb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, kbcb_to_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "kbcb_to_raw");
	request1.put ("amount", "1");
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::kbcb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, kbcb_from_raw)
{
	rai::system system (24000, 1);
	rai::node_init init1;
	auto & node1 (*system.nodes[0]);
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "kbcb_from_raw");
	request1.put ("amount", rai::kbcb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, bcb_to_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "bcb_to_raw");
	request1.put ("amount", "1");
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (rai::bcb_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, bcb_from_raw)
{
	rai::system system (24000, 1);
	rai::node_init init1;
	auto & node1 (*system.nodes[0]);
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "bcb_from_raw");
	request1.put ("amount", rai::bcb_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc, system.service);
	while (response1.status == 0)
	{
		system.poll ();
...
TEST (rpc, block_create_state)
{
	...
	request.put ("balance", (rai::genesis_amount - rai::Gbcb_ratio).convert_to<std::string> ());
...
TEST (rpc, block_create_state_open)
{
	...
	rai::genesis genesis;
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (rai::test_genesis_key.pub, key.pub, rai::Gbcb_ratio));
	...
	request.put ("representative", rai::test_genesis_key.pub.to_account ());
	request.put ("balance", rai::Gbcb_ratio.convert_to<std::string> ());
...
TEST (rpc, confirmation_history)
{
	rai::system system (24000, 1);
	system.wallet (0)->insert_adhoc (rai::test_genesis_key.prv);
	auto block (system.wallet (0)->send_action (rai::test_genesis_key.pub, rai::test_genesis_key.pub, rai::Gbcb_ratio));
	...
	assert (tally_num == rai::genesis_amount || tally_num == (rai::genesis_amount - rai::Gbcb_ratio));
	system.stop ();
}
