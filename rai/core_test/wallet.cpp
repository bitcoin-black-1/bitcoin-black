...
TEST (wallet, send_race)
{
	...
	for (auto i (1); i < 60; ++i)
	{
		ASSERT_NE (nullptr, system.wallet (0)->send_action (rai::test_genesis_key.pub, key2.pub, rai::Gbcb_ratio));
		ASSERT_EQ (rai::genesis_amount - rai::Gbcb_ratio * i, system.nodes[0]->balance (rai::test_genesis_key.pub));
	}
}
