...
receive_minimum (rai::bcb_ratio),
online_weight_minimum (60000 * rai::Gbcb_ratio),
...
lmdb_max_dbs (128)
{
	...
	switch (rai::bitcoinblack_network)
	{
		case rai::bitcoinblack_networks::bitcoinblack_test_network:
			preconfigured_representatives.push_back (rai::genesis_account);
			break;
		case rai::bitcoinblack_networks::bitcoinblack_beta_network:
			preconfigured_peers.push_back ("127.0.0.1");
			...
			break;
		case rai::bitcoinblack_networks::bitcoinblack_live_network:
			preconfigured_peers.push_back ("18.233.201.235");
			//preconfigured_peers.push_back ("bitcoinblack.info");
			preconfigured_representatives.push_back (rai::account ("C8957851C73D1AF60BE7A95434DF74EB6133D1CD7FA2D9B5AE8A90EB912AA0D4"));
			preconfigured_representatives.push_back (rai::account ("F92A1E8C1A08BC2D8EA9E4E67B26130FD3A94C23F780051DB7EE7D6F3574D3BE"));
			preconfigured_representatives.push_back (rai::account ("C0B487630DF9C57C4B16ED9F777CC423BFC872010E142314F3DAA33F35358E2E"));
			preconfigured_representatives.push_back (rai::account ("5831BA3C4F761B47FA9526DA3FF933320234767C17277484E42F970999BDDFDC"));
			preconfigured_representatives.push_back (rai::account ("B820813B10BF8751A68BB2BA2A83AAAA75D7F5BC17A931929BE0D8803BDAD899"));
			preconfigured_representatives.push_back (rai::account ("008EB0FCCC5EE7DF7731BE50068C13FA987A1210C871201EA42373CC3694637E"));
			preconfigured_representatives.push_back (rai::account ("D3A350CFD492CAFDEDA7CDB34E740E85AD460C7C7A8846DA9FA4D9B9425BB36C"));
			preconfigured_representatives.push_back (rai::account ("003A467C272FB80363485278AA4F02D105C9F1420A9EFA4FB250EBCCA500EDC7"));
...
bool rai::node_config::upgrade_json (unsigned version, boost::property_tree::ptree & tree_a)
{
	...
	switch (version)
	{
		...
		case 3:
			...
			tree_a.put ("receive_minimum", rai::bcb_ratio.convert_to<std::string> ());
			tree_a.erase ("version");
			tree_a.put ("version", "4");
			result = true;
		case 4:
...
stats (config.stat_config)
{
	...
	if (rai::bitcoinblack_network == rai::bitcoinblack_networks::bitcoinblack_live_network || rai::bitcoinblack_network == rai::bitcoinblack_networks::bitcoinblack_beta_network)
	{
		...
		if (existing != blocks.get<1> ().end ())
		{
			...
			if (is_new)
			{
				...
				if (tally > bootstrap_threshold (transaction))
				{
					...
					auto now (std::chrono::steady_clock::now ());
					node.alarm.add (rai::bitcoinblack_network == rai::bitcoinblack_networks::bitcoinblack_test_network ? now + std::chrono::milliseconds (5) : now + std::chrono::seconds (5), [node_l, hash]() {
...
{
	assert (balance_a >= amount_a * rai::Gbcb_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= rai::Gbcb_ratio;
		auto balance_scaled ((balance_l / rai::Mbcb_ratio).convert_to<double> ());
...
void rai::online_reps::vote (std::shared_ptr<rai::vote> const & vote_a)
{
	...
	if (rep_it == reps.get<1> ().end ())
	{
		...
		if (online_stake_total < old_stake)
		{
			...
			online_stake_total = rai::genesis_amount;	// Mstar
		}
		reps.insert (info);
	}
	else
	...
	if (!result)
	{
		if (version_a >= rai::protocol_version_min)
		{
			...
			else
			{
				...
				if (!result && bitcoinblack_network != bitcoinblack_networks::bitcoinblack_test_network)
				{
...
bool rai::reserved_address (rai::endpoint const & endpoint_a, bool blacklist_loopback)
{
	...
	else if (rai::bitcoinblack_network == rai::bitcoinblack_networks::bitcoinblack_live_network)
	{
...
rai::election_vote_result rai::election::vote (rai::account rep, uint64_t sequence, rai::block_hash block_hash)
{
	...
	if (rai::bitcoinblack_network == rai::bitcoinblack_networks::bitcoinblack_test_network || weight > supply / 1000) // 0.1% or above
	{
...
{
	if (rai::bitcoinblack_network != rai::bitcoinblack_networks::bitcoinblack_test_network)
	{
		std::lock_guard<std::mutex> lock (mutex);
