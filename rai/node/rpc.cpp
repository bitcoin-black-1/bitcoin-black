...
void rai::rpc_handler::mbcb_from_raw (rai::uint128_t ratio)
{
...
void rai::rpc_handler::mbcb_to_raw (rai::uint128_t ratio)
{
	auto amount (amount_impl ());
	if (!ec)
	{
...
void rai::rpc_handler::version ()
{
	...
	response_l.put ("node_vendor", boost::str (boost::format ("BitcoinBlack %1%.%2%") % RAIBLOCKS_VERSION_MAJOR % RAIBLOCKS_VERSION_MINOR));
	response_errors ();
}
...
void rai::rpc_handler::process_request ()
{
	try
	{
		...
		else
		{
			...
			else if (action == "kbcb_from_raw")
			{
				mbcb_from_raw (rai::kbcb_ratio);
			}
			else if (action == "kbcb_to_raw")
			{
				mbcb_to_raw (rai::kbcb_ratio);
			}
			else if (action == "ledger")
			{
				ledger ();
			}
			else if (action == "mbcb_from_raw")
			{
				mbcb_from_raw ();
			}
			else if (action == "mbcb_to_raw")
			{
				mbcb_to_raw ();
			}
			else if (action == "password_change")
			...
			else if (action == "bcb_from_raw")
			{
				mbcb_from_raw (rai::bcb_ratio);
			}
			else if (action == "bcb_to_raw")
			{
				mbcb_to_raw (rai::bcb_ratio);
			}
