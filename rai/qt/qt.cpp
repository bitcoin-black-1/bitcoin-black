...
bool bitcoinblack_qt::eventloop_processor::event (QEvent * event_a)
{
	assert (dynamic_cast<bitcoinblack_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<bitcoinblack_qt::eventloop_event *> (event_a)->action ();
	return true;
}

bitcoinblack_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

bitcoinblack_qt::self_pane::self_pane (bitcoinblack_qt::wallet & wallet_a, rai::account const & account_a) :
window (new QWidget),
layout (new QVBoxLayout),
self_layout (new QHBoxLayout),
self_window (new QWidget),
your_account_label (new QLabel ("Your BitcoinBlack account:")),
...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("Version %1%.%2%") % BLOCKCHAIN_VERSION_MAJOR % BLOCKCHAIN_VERSION_MINOR).c_str ());
...
void bitcoinblack_qt::self_pane::refresh_balance ()
{
...
bitcoinblack_qt::accounts::accounts (bitcoinblack_qt::wallet & wallet_a) :
window (new QWidget),
...
void bitcoinblack_qt::accounts::refresh_wallet_balance ()
{
	auto transaction (this->wallet.wallet_m->wallets.tx_begin_read ());
...
void bitcoinblack_qt::accounts::refresh ()
{
...
bitcoinblack_qt::import::import (bitcoinblack_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
bitcoinblack_qt::history::history (rai::ledger & ledger_a, rai::account const & account_a, bitcoinblack_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
void bitcoinblack_qt::history::refresh ()
{
...
bitcoinblack_qt::block_viewer::block_viewer (bitcoinblack_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void bitcoinblack_qt::block_viewer::rebroadcast_action (rai::uint256_union const & hash_a)
{
	auto done (true);
...
bitcoinblack_qt::account_viewer::account_viewer (bitcoinblack_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this]() {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (BCB): ") + wallet.format_balance (balance.first));
			if (!balance.second.is_zero ())
			{
				final_text += "\nPending: " + wallet.format_balance (balance.second);
			}
...
bitcoinblack_qt::stats_viewer::stats_viewer (bitcoinblack_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
void bitcoinblack_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
bitcoinblack_qt::status::status (bitcoinblack_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (bitcoinblack_qt::status_types::nominal);
	set_text ();
}

void bitcoinblack_qt::status::erase (bitcoinblack_qt::status_types status_a)
{
	assert (status_a != bitcoinblack_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void bitcoinblack_qt::status::insert (bitcoinblack_qt::status_types status_a)
{
	assert (status_a != bitcoinblack_qt::status_types::nominal);
...
std::string bitcoinblack_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case bitcoinblack_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case bitcoinblack_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case bitcoinblack_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case bitcoinblack_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case bitcoinblack_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case bitcoinblack_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case bitcoinblack_qt::status_types::nominal:
...
std::string bitcoinblack_qt::status::color ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
bitcoinblack_qt::wallet::wallet (QApplication & application_a, bitcoinblack_qt::eventloop_processor & processor_a, rai::node & node_a, std::shared_ptr<rai::wallet> wallet_a, rai::account & account_a) :
rendering_ratio (rai::Mbcb_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
void bitcoinblack_qt::wallet::start ()
{
	std::weak_ptr<bitcoinblack_qt::wallet> this_w (shared_from_this ());
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (bitcoinblack_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (bitcoinblack_qt::status_types::active);
					}
				}
			}));
		}
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (bitcoinblack_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (bitcoinblack_qt::status_types::synchronizing);
					}
				}
			}));
		}
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (bitcoinblack_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (bitcoinblack_qt::status_types::working);
					}
				}
			}));
		}
...
void bitcoinblack_qt::wallet::refresh ()
{
	{
...
void bitcoinblack_qt::wallet::update_connected ()
{
	if (node.peers.empty ())
	{
		active_status.insert (bitcoinblack_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (bitcoinblack_qt::status_types::disconnected);
	}
}

void bitcoinblack_qt::wallet::empty_password ()
{
...
void bitcoinblack_qt::wallet::change_rendering_ratio (rai::uint128_t const & rendering_ratio_a)
{
...
std::string bitcoinblack_qt::wallet::format_balance (rai::uint128_t const & balance) const
{
	auto balance_str = rai::amount (balance).format_balance (rendering_ratio, 0, false);
	auto unit = std::string ("BCB");
	if (rendering_ratio == rai::kbcb_ratio)
	{
		unit = std::string ("kbcb");
	}
	else if (rendering_ratio == rai::bcb_ratio)
	{
		unit = std::string ("bcb");
	}
...
void bitcoinblack_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void bitcoinblack_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

bitcoinblack_qt::settings::settings (bitcoinblack_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
void bitcoinblack_qt::settings::refresh_representative ()
{
...
void bitcoinblack_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void bitcoinblack_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (bitcoinblack_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (bitcoinblack_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (bitcoinblack_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (bitcoinblack_qt::status_types::vulnerable);
	}
}

bitcoinblack_qt::advanced_actions::advanced_actions (bitcoinblack_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
mbcb (new QRadioButton ("Mbcb")),
kbcb (new QRadioButton ("kbcb")),
rai (new QRadioButton ("bcb")),
back (new QPushButton ("Back")),
...
bootstrap_label (new QLabel ("IPV6:port \"::ffff:192.168.0.1:7078\"")),
...
wallet (wallet_a)
{
	ratio_group->addButton (mbcb);
	ratio_group->addButton (kbcb);
	ratio_group->addButton (rai);
	ratio_group->setId (mbcb, 0);
	ratio_group->setId (kbcb, 1);
	ratio_group->setId (rai, 2);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (mbcb);
	scale_layout->addWidget (kbcb);
	...
	QObject::connect (mbcb, &QRadioButton::toggled, [this]() {
		if (mbcb->isChecked ())
		{
			this->wallet.change_rendering_ratio (rai::Mbcb_ratio);
		}
	});
	QObject::connect (kbcb, &QRadioButton::toggled, [this]() {
		if (kbcb->isChecked ())
		{
			this->wallet.change_rendering_ratio (rai::kbcb_ratio);
		}
	});
	QObject::connect (rai, &QRadioButton::toggled, [this]() {
		if (rai->isChecked ())
		{
			this->wallet.change_rendering_ratio (rai::bcb_ratio);
		}
	});
	mbcb->click ();
...
void bitcoinblack_qt::advanced_actions::refresh_peers ()
{
...
void bitcoinblack_qt::advanced_actions::refresh_ledger ()
{
...
void bitcoinblack_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

bitcoinblack_qt::block_entry::block_entry (bitcoinblack_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
bitcoinblack_qt::block_creation::block_creation (bitcoinblack_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void bitcoinblack_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void bitcoinblack_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void bitcoinblack_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void bitcoinblack_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void bitcoinblack_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
...
void bitcoinblack_qt::block_creation::create_send ()
{
	rai::account account_l;
...
void bitcoinblack_qt::block_creation::create_receive ()
{
...
void bitcoinblack_qt::block_creation::create_change ()
{
	rai::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
...
void bitcoinblack_qt::block_creation::create_open ()
{
	rai::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
