...
{
	bitcoinblack_qt::eventloop_processor processor;
	rai::system system (24000, 1);
	auto wallet_l (system.nodes[0]->wallets.create (rai::uint256_union ()));
	auto key (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<bitcoinblack_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key));
	wallet->start ();
...
TEST (wallet, status)
{
	...
	auto wallet (std::make_shared<bitcoinblack_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub));
	wallet->start ();
	auto wallet_has = [wallet](bitcoinblack_qt::status_types status_ty) {
	...
	ASSERT_FALSE (wallet_has (bitcoinblack_qt::status_types::synchronizing));
	system.deadline_set (25s);
	while (!wallet_has (bitcoinblack_qt::status_types::synchronizing))
	{
	...
	ASSERT_TRUE (wallet_has (bitcoinblack_qt::status_types::disconnected));
}
...
TEST (wallet, startup_balance)
{
	...
	auto wallet (std::make_shared<bitcoinblack_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub));
	wallet->start ();
	ASSERT_EQ ("Balance: 0 BCB", wallet->self.balance_label->text ().toStdString ());
}
...
TEST (wallet, select_account)
{
	...
	auto wallet (std::make_shared<bitcoinblack_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key1));
	wallet->start ();
...
TEST (wallet, password_change)
{
	...
	auto wallet (std::make_shared<bitcoinblack_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->settings_button, Qt::LeftButton);
	{
		auto transaction (system.nodes[0]->store.tx_begin ());
...
TEST (history, short_text)
{
	...
	bitcoinblack_qt::history history (ledger, rai::test_genesis_key.pub, *wallet);
	history.refresh ();
...
TEST (wallet, import)
{
	...
	auto wallet (std::make_shared<bitcoinblack_qt::wallet> (*test_application, processor, *system.nodes[1], system.wallet (1), key2.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, ignore_empty_adhoc)
{
	...
	rai::keypair key1;
	system.wallet (0)->insert_adhoc (key1.prv);
	auto wallet (std::make_shared<bitcoinblack_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, change_seed)
{
	...
	auto wallet (std::make_shared<bitcoinblack_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), wallet_key));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, seed_work_generation)
{
	...
	rai::system system (24000, 1);
	auto key1 (system.wallet (0)->deterministic_insert ());
	auto wallet (std::make_shared<bitcoinblack_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, DISABLED_synchronizing)
{
	...
	auto wallet (std::make_shared<bitcoinblack_qt::wallet> (*test_application, processor, *system0.nodes[0], system0.wallet (0), key1));
	wallet->start ();
	{
	...
	ASSERT_EQ (0, wallet->active_status.active.count (bitcoinblack_qt::status_types::synchronizing));
	system0.nodes[0]->bootstrap_initiator.bootstrap (system1.nodes[0]->network.endpoint ());
	system1.deadline_set (10s);
	while (wallet->active_status.active.count (bitcoinblack_qt::status_types::synchronizing) == 0)
	{
	...
	while (wallet->active_status.active.count (bitcoinblack_qt::status_types::synchronizing) == 1)
	{
		ASSERT_NO_ERROR (system0.poll ());
		ASSERT_NO_ERROR (system1.poll ());
		test_application->processEvents ();
