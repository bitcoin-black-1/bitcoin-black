...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("debug_profile_process", "Profile active blocks processing (only for bitcoinblack_test_network)")
	...
	if (ec == rai::error_cli::unknown_command)
	{
		...
		{
			bitcoinblack_daemon::daemon daemon;
		...
		else if (vm.count ("debug_profile_process"))
		{
			...
			{
				std::cerr << "For this test ACTIVE_NETWORK should be bitcoinblack_test_network" << std::endl;
			}
		}
