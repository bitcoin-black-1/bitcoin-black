...
{
	QMessageBox message (QMessageBox::Critical, "Error starting BitcoinBlack", message_a.c_str ());
...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path)
{
	...
	if (!error)
	{
		...
		std::shared_ptr<bitcoinblack_qt::wallet> gui;
		...
		if (!init.error ())
		{
			...
			application.postEvent (&processor, new bitcoinblack_qt::eventloop_event ([&]() {
				gui = std::make_shared<bitcoinblack_qt::wallet> (application, processor, *node, wallet, config.account);
				splash->close ();
				gui->start ();
